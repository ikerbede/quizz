...
import { QuestionMultiple } from '../../shared/question.model';
...
export class QuestionMultipleComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.multipleOptions = this.question.choices.map((choice) => ({
      label: choice,
      selected: false,
    }));
  }
  ...
  validate(): void {
    const selectedOptions = this.multipleOptions.filter(
